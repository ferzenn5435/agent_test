...
| --- | --- | --- | --- | --- |
| `default` | `DEFAULT_LLM_BASE_URL` | `DEFAULT_LLM_API_KEY` | `DEFAULT_LLM_MODEL` | 默认 profile |
...
推荐复制 `.env.example` 为 `.env`，然后在 `.env` 中填写本地配置。`default` 使用专属环境变量名称：

```env
DEFAULT_LLM_BASE_URL=https://api.openai.com/v1
DEFAULT_LLM_API_KEY=your_api_key
DEFAULT_LLM_MODEL=your_model
```
...
```powershell
$env:DEFAULT_LLM_BASE_URL = "https://api.openai.com/v1"
$env:DEFAULT_LLM_API_KEY = "your_api_key"
$env:DEFAULT_LLM_MODEL = "your_model"
...
```powershell
$env:DEFAULT_LLM_BASE_URL = "https://example.com/v1/chat/completions"
```
...
替换不同模型供应商时，优先只修改 `llm_client.py` 和对应 provider，保持 `LlmClient.chat_response(messages)` 返回包含 `content`、`profile_name`、`provider`、`model`、`usage` 和 `latency_ms` 的结构化响应即可。
<!-- v0.6.1 Pending Approval -->
